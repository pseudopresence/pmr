...
N=N/T;
  ...
  if (K<=p) 
    ...
    fprintf('Initializing with Factor Analysis...\n');
    [L,mu,Ph,LM]=fa(X,K,100,0.001);
    i=length(LM);
    C=L;
    R=Ph;
    Phi=diag(1./R);
...
for cycle=1:cyc
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  LL=[LL lik];
  fprintf('Cycle %4d  Log likelihood %11.6f',cycle,lik);
  
  if (cycle<=2)
    likbase=lik;
  elseif (lik<oldlik) 
    fprintf(' violation');
  elseif ((lik-likbase)<(1 + tol)*(oldlik-likbase)|isnan(lik)) 
    fprintf('\n');
    break;
  end;
  fprintf('\n');
